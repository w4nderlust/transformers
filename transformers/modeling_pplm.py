...
from .modeling_gpt2_for_pplm import GPT2LMHeadModel, GPT2Tokenizer, \
...
def latent_perturb(model, args, context=None, sample=True, device='cuda'):
    if args.discrim == 'clickbait':
        ...
        classifier.load_state_dict(
            torch.load("discrim_models/clickbait_classifierhead.pt"))
        classifier.eval()
        ...
        classifier = ClassificationHead(class_size=5, embed_size=1024).to(
            device)
        classifier.load_state_dict(
            torch.load("discrim_models/sentiment_classifierhead.pt"))
        classifier.eval()
        ...
        classifier = ClassificationHead(class_size=2, embed_size=1024).to(
            device)
        classifier.load_state_dict(
            torch.load("discrim_models/toxicity_classifierhead.pt"))
        classifier.eval()
